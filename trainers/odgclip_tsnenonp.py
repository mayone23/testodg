...
class StableDiffusion(nn.Module):
    ...
    def forward(self, images, pos_prompt):       
        generated_images = []
        ...
        with torch.no_grad():
            for i in range(batchsize):
                batch_output = self.pipe(prompt=positive_prompt[i], guidance_scale=15)
                generated_images.append(batch_output.images[0])
        generated_images = torch.stack([ToTensor()(img) for img in generated_images]).to(device)
        return generated_images
...
class GenerateUnknownImages(nn.Module):
    ...
    def forward(self, image, pos_prompt):
        ...
        generated_unknown_images = self.diffusion(image, pos_prompt) 
